 ...
 *
 * @ViewsFilter("civicrm_entity_civicrm_activity_contact_record_filter")
 */
class CivicrmEntityCivicrmActivityContactRecordFilter extends NumericFilter {
  ...
  public function query() {
    ...
    $info = $this
      ->operators();
    if (!empty($info[$this->operator]['method']) && self::RECORD_TYPE_MAPPING[$this->realField]) {
      $condition = new Condition('AND');
      $condition->condition($field, $this->value['value'], $this->operator);
      $condition->condition("$civicrm_activity_contact_table_alias.record_type_id", self::RECORD_TYPE_MAPPING[$this->realField]);
